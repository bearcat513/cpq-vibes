...
export function LineConfigurator({ product, line, currency, locale, quoteTermMonths, onSave, onClose }: Props) {
  ...
  const step = (delta: number) =>
    set({ quantity: Math.max(product.minQuantity || 1, draft.quantity + delta) });
    ...
    >
      <div className="space-y-4">
        <div className="grid gap-3 @md:grid-cols-2 @2xl:grid-cols-3">
          <Field label={`Quantity (${product.unitOfMeasure})`}>
            <div className="flex items-center gap-1">
              <Button variant="outline" size="icon-sm" onClick={() => step(-1)} aria-label="One fewer">
                <Minus />
              </Button>
              <Input
                type="number"
              ...
              />
              <Button variant="outline" size="icon-sm" onClick={() => step(1)} aria-label="One more">
                <Plus />
              </Button>
            </div>
          </Field>
        ...
        {product.optionGroups.map(group => {
          const chosen = group.options.filter(option => draft.selectedOptions.includes(option.key)).length;
          return (
