 ...
 * ## Why it is portalled
 *
 * "Full height of the window" is only true of a `fixed` element while nothing
 * above it has a transform. Every screen in the app is rendered inside an
 * entrance animation, so a panel left where it was written is fixed to that
 * screen's box and ends at the bottom of the content. See `Portal`.
 */
  ...
  return (
    <Portal>
      <div className="fixed inset-0 z-50 flex justify-end">
        ...
        <div
          className="animate-in fade-in absolute inset-0 bg-[var(--scrim)] backdrop-blur-[1px] duration-200"
          onClick={onClose}
          aria-hidden="true"
        />
        ...
        <aside
          ...
          aria-label={title}
          className={cn(
            "panel relative flex h-full w-full flex-col border-l shadow-[-24px_0_48px_-24px_var(--cast)]",
        ...
        >
          <header className="seam bg-card/60 flex shrink-0 items-start justify-between gap-3 border-b px-4 py-3">
